...
Создайте новый NSMP Embedded App одной командой. CLI задаст необходимые вопросы прямо в терминале, скопирует готовый шаблон и при необходимости установит зависимости. Из корня существующего Vue + Vite проекта он предложит вместо этого подключить NSMP-пакеты и утилиты.
...
- выбор полезных утилит NSMP при создании или инициализации проекта;
- подключение дополнительных пакетов через интерактивный CLI.

CLI также умеет подключать эти зависимости и утилиты к уже существующему проекту на Vue + Vite. Запустите команду из корня такого проекта и подтвердите инициализацию: CLI обновит `package.json` и добавит выбранные папки в `src/utils`, не изменяя исходный код приложения.
...
<a href="https://erilovnikita.github.io/create-nsmp-embedded-app/">
