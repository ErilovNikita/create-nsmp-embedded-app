...
Workflow публикации запускается автоматически после merge в `main` или вручную через GitHub Actions → `Publish package to npm` → `Run workflow`.

Перед публикацией увеличьте версию пакета:
...
Workflow проверяет CLI, содержимое npm-пакета и наличие версии в registry. Уже опубликованная версия повторно не публикуется.

Для публикации без npm-токена в настройках npm package должен быть создан Trusted Publisher для репозитория и файла `.github/workflows/npm-publish.yml`.
